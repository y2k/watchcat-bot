...
let try_ban env user_id bar_user_id =
  ...
  if IntSet.mem user_id state.trusted_users then
    ...
    | Some reg_date ->
        ...
        if reg_duration < 2.0 *. 24.0 *. 3600.0 then `KickUser bar_user_id
        else
          `SendMessage
            (Printf.sprintf
    ...
    `SendMessage
      "Извините, но вас нет в списке \
       доверенных пользователей. Обратитесь \
       пожалуйста к админам, для добавления в \
       список."
